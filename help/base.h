...
x,y   屏幕 显示位置
mw		图片的原始宽度，即完整宽度。
rop :  (TRANS_NONE至TRANS_MIRROR_ROT270,可与BM_TRANS和BM_COPY组合使用功能与mrc_bitmapshowEx相同)
sx,sy           源图片的起始位置
w,h             欲加载图片的宽高
...
返回:
      ...
      MR_FAILED         失败
*/
int32 bitmapshowextrans(uint16 *p, int16 x, int16 y, int16 mw, int16 w, int16 h, uint16 top, int16 sx, int16 sy, uint16 transcolor);
